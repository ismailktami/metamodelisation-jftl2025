...
// Pour chaque configuration, il teste le cas où tous les champs sont valides.
...
formConfigs.forEach(formConfig=>{
  // Filtrage des règles pour ne garder que celles de type 'ALL_FIELDS_VALID'
  for (const rule of formConfig.rules.filter(rule => rule.conditionType === 'ALL_FIELDS_VALID')) {
    ...
    test(`[${formConfig.formName}][ALL_FIELDS_VALID] ${actionsTitle}`, async ({ page }) => {
      await page.goto(formConfig.formLink);
      let inputs = {};
      // Remplit tous les champs avec des valeurs valides générées automatiquement
